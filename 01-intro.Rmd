...
Para contestar estas preguntas se pueden usar métodos de **análisis de regresión múltiple**, que casi siempre se hacen con un programa de cómputo, en nuestro caso, utilizando R. Este capítulo contiene una breve introducción al análisis de regresión múltiple y a la difícil tarea de construcción de 
modelos, es decir, escoger el modelo correcto para una aplicación práctica.

Para mas información leerse la sección 13.1 del libro de  @mendenhall2010introduccion 
...
```{r eval=FALSE}
packages <- c(
 'bookdown', 'knitr', 'rmarkdown','scatterplot3d','plotly', 'tidyverse','rgl', 'lmtest','car', 'psych', 'DT', 'broom','kableExtra', 'GGally','plot3D', 'plot3Drgl','equatiomatic','performance','tinytex' 
)
...
El método de mínimos cuadrados ordinarios puede emplearse para estimar los coeficientes de regresión del modelo lineal múltiple de la ecuación anterior \@ref(eq:rlm).
...
library(kableExtra)
knitr::kable(cbind(Observaciones = observaciones,datos),booktabs = TRUE, align = "c", caption = "Tabla con los datos de la regresion  multiple para Hop Scotch Airlines" ) 
```
...
```{r explorscatter, fig.cap="Grafico de dispersion 3D entre las variables con scatterplot3d", echo=TRUE}
...
El mismo diagrama de dispersión se puede crear usando el paquete `plotly`. El cual permite al lector muchas mas posibilidades con el diagrama como moverlo, acercarlo, etc.

```{r explorplotly, fig.cap="Grafico de dispersion 3D entre las variables con plotly"}
library(plotly)
...
```{r multihist, fig.cap="Histograma multiple con psych"} 
library(psych)
...
Esta matriz también sirve para explorar los datos al comparar varios subconjuntos de la base de datos, pero adiciona la correlación entre cada uno de los subconjuntos. Para obtenerla con R se utilizo el paquete `GGally` que ayuda a hacer este tipo de gráficos.

```{r explorvar, fig.cap="Matriz de dispersión con correlaciones con GGally"}
library(GGally)
...
**Solución sin utilizar la funcion lm()**:
...
Otra forma de obtener las ecuación del modelo de regresion múltiple en formato latex y html es utilizando el paquete `equationmatic`

```{r}
library(equatiomatic)

#Devuelve la ecuación del modelo ajustada
extract_eq(modelo)

#Devolver coeficientes numéricos reales en lugar de letras griegas con:use_coefs = TRUE
...
```{r}
y_transpuesta_y <- t(pasajeros) %*% pasajeros
...
inv_x_transpuesta_x
matr_estimaciones_beta <- solve(x_transpuesta_x) %*% x_transpuesta_y
matr_estimaciones_beta
```
...
knitr::kable(datos_t,booktabs = TRUE, align = "c", caption = "Tabla de Residuos y Ajustes" ) 
```
...
Una forma de evaluar si el modelo creado es un buen modelo es utilizando el paquete `performance`

```{r performance}
library(performance)
knitr::kable(model_performance(modelo),caption = "Tabla de Desempeño del Modelo" )
```
...
$$ $$
\boldsymbol{\hat{\beta}'X'Y} =\begin{bmatrix}
...
\begin{bmatrix}
 ...
 2.40&  2.72&  \cdots&  2.17
\end{bmatrix} \\
\boldsymbol{\hat{\beta}'X'Y} = 5266.256
$$ 

Por lo tanto, la suma de cuadrados del error es:

$$
SCE = \boldsymbol{Y'Y - \hat{\beta}'X'Y}=93.74405
$$ Por consiguiente, la estimación de $\sigma^{2}$ es:

$$
\hat{\sigma}^{2}=CME=\frac{SCE}{n-p}=\frac{93.74405}{15-3}=7.812004
$$
```{r}
# Número de predictores (regresores)
p <- 3

# Producto de la transpuesta de beta y x_transpuesta_y
producto_transpuesta_beta_y <- t(matr_estimaciones_beta) %*% x_transpuesta_y
producto_transpuesta_beta_y

# Suma de cuadrados de los residuos
suma_cuadrados_residuos <- y_transpuesta_y - producto_transpuesta_beta_y
suma_cuadrados_residuos

# Varianza estimada del error (s^2)
sce <- suma_cuadrados_residuos / (n - p)
sce
```
...
Todas las demostraciones matemáticas utilizadas en el libro son profundizadas en el capitulo 13 del libro de @wackerly2002estadistica .
